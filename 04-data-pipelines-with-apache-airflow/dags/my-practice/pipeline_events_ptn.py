...
def _extract_data(ds):
    ...
    # print(os.getcwd())
    with open(f"{MY_PRACTICE_FOLER}/data/{DATA}-{ds}.csv", "w") as file:
        header = data[0].keys()
...
def _load_data_to_gcs(ds):
    ...
    src_path = f"{MY_PRACTICE_FOLER}/data/{DATA}-{ds}.csv"
    destination_path = f"pipeline-practice/{BUSINESS_DOMAIN}/{DATA}/{ds}/{DATA}-{ds}.csv"
...
with DAG(
    dag_id="pipeline_events_ptn",  # Replace xxx with the data name
